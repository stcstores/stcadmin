...
def refund(
    ...
    refund = lost_in_post_refund_factory.create(returned=False)
...
@pytest.fixture
def url(db, refund):
    return f"/orders/refund/{refund.id}/set_returned/"
...
@pytest.mark.django_db
def test_marks_returned(refund, valid_get_response):
