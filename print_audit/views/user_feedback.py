...
class UserFeedback(PrintAuditUserMixin, TemplateView):
    ...
    def get_pack_counts(self):
        ...
        self.pack_counts = {
            user.id: self.orders.filter(user=user).count() for user in self.users
        }
    ...
    def get_feedback_counts(self):
        ...
        for user in self.users:
            ...
            for feedback_type in self.feedback_types:
                self.feedback_counts[user.id][feedback_type.id] = self.feedback.filter(
                    user=user, feedback_type=feedback_type
                ).count()
