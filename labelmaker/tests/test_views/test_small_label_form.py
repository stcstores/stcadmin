...
@pytest.fixture
def path():
    return "labelmaker:small_labels"
...
def test_has_form(html):
    target = reverse("labelmaker:small_label_pdf")
    assert html.find(f'form[action="{target}"][method="post"]')


@pytest.mark.django_db
def test_has_label_text(html):
    assert html.find('textarea[name="label_text"]')


@pytest.mark.django_db
def test_has_quantity_input(html):
    assert html.find('input[name="quantity"]')
