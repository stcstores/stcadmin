...
@pytest.fixture
def url(country, staff):
    return reverse("purchases:create_shipping_purchase")
...
def country(country_factory):
    return country_factory.create(name="United Kingdom")


@pytest.fixture
def mock_get_form(mock_form):
    with mock.patch("purchases.views.CreateShippingPurchase.get_form") as mock_get_form:
...
def test_uses_template(get_response):
    assert "purchases/create_shipping_purchase.html" in (
        t.name for t in get_response.templates
    )


@pytest.mark.django_db
def test_get_initial(staff, country, get_response):
    form = get_response.context["form"]
    assert form.initial == {"purchaser": staff.id, "country": country}
