...
from purchases.forms import UpdatePurchaseForm
...
def test_updates_quantity(purchase):
    form = UpdatePurchaseForm({"quantity": 5}, instance=purchase)
...
def test_update_stock_level_method(
    ...
    form = UpdatePurchaseForm({"quantity": new_quantity}, instance=purchase)
    assert form.is_valid()
    form.save()
...
def test_update_stock_level_method_raises_for_stock_less_than_zero(
    ...
    form = UpdatePurchaseForm({"quantity": 16}, instance=purchase)
    assert form.is_valid()
    form.save()
    with pytest.raises(Exception):
        form.update_stock_level()
