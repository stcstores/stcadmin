...
@pytest.fixture
def url(shipment_package, shipment_item):
    return reverse("fba:delete_package", args=[shipment_package.pk])
...
def test_uses_template(get_response):
    assert "fba/shipments/confirm_delete_package.html" in (
...
@pytest.mark.django_db
def test_shipment_order_in_context(get_response, shipment_package):
    assert get_response.context["object"] == shipment_package


@pytest.mark.django_db
def test_deletes_shipment_order(post_response, shipment_package, shipment_item):
    assert (
...
@pytest.mark.django_db
def test_redirects(post_response, shipment_package):
    assert post_response.status_code == 302
    assert post_response["Location"] == reverse(
        "fba:update_shipment", args=[shipment_package.shipment_order.pk]
    )
