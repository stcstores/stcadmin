...
@pytest.fixture
def url():
    return reverse("fba:create_shipment_create_destination")
...
def test_uses_template(get_response):
    assert "fba/shipments/create_shipment/create_destination.html" in [
...
@pytest.mark.django_db
def test_redirect(post_response, form_data):
    destination = FBAShipmentDestination.objects.get(name=form_data["name"])
    assert post_response.status_code == 302
    assert post_response["Location"] == reverse(
        "fba:create_shipment", kwargs={"destination_pk": destination.pk}
    )
