...
@pytest.fixture
def url():
    return reverse("fba:api_download_shipment_file")
...
@pytest.mark.django_db
def test_filename(export, post_response):
    assert (
...
def test_content(export, post_response):
    assert post_response.content == export.generate_export_file().encode("utf8")
